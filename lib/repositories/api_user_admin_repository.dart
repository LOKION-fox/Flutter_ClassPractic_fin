...
class ApiUserAdminRepository implements UserAdminRepository {
  ...
  Future<List<AppUser>> findUsers() {
    return _auth.authorized(() => _api.run(() async {
          final records = await _api.pocketBase.collection('users').getFullList(
                sort: 'username',
              );
          return records.map((record) => AppUser.fromJson(record.toJson())).toList();
        }));
  }
  ...
  Future<AppUser> changeRole(String userId, AppRole role) {
    return _auth.authorized(() => _api.run(() async {
          final record = await _api.pocketBase.collection('users').update(
                userId,
  ...
  Future<int> _count(String collection, {String? filter}) async {
    final result = await _api.pocketBase.collection(collection).getList(
          page: 1,
          perPage: 1,
          filter: filter,
        );
